#!/bin/bash
# v2: chunks of 10; after each chunk: stop daemon (memory), delete per-flavor intermediates (disk), release, drop released APKs (disk).
cd /root/claude/Factory
while true; do
  todo=$(for p in $(find specs -name '*.json' -mmin +15); do id=$(basename $p .json); [ -f dist/$id-v1.0.aab ] || echo "f$(echo $id | tr -d '_')"; done | head -10)
  n=$(echo "$todo" | grep -c .)
